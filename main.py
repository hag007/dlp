...
dataset = CMAP(replications=args.reps)
...
for i, (train, valid, test, contfeats, binfeats) in enumerate(dataset.get_train_valid_test()):
    ...
    perm = list(binfeats) + list(contfeats)
    ...
    p_y_zt_dist = p_y_zt(dim_in=args.z_dim, nh=3, dim_h=args.h_dim, dim_x=len(binfeats), dim_out=1).cuda()
    ...
    for epoch in range(n_epoch):
        ...
        for j in range(int(len(idx)/M)):
            # select random batch
            batch = idx[j*M:np.min([(j+1)*M,len(idx)])] # np.random.choice(idx, M)
            ...
            y = p_y_zt_dist(z_infer_sample, t_train, x_bin.sample())
        ...
        if epoch % args.print_every == 0:
            print('Epoch %i' % epoch)
            y0, y1 = get_y0_y1(p_y_zt_dist, q_y_xt_dist, q_z_tyx_dist, p_x_z_dist, torch.tensor(xalltr).cuda(),
            ...
            print('Training set - rmse factual: %f' %rmses_train)
            rmses_trains.append(rmses_train)



            y0, y1 = get_y0_y1(p_y_zt_dist, q_y_xt_dist, q_z_tyx_dist, p_x_z_dist, torch.tensor(xte).cuda(),
                               torch.tensor(tte).cuda())

            yt0, yt1 = get_treated_y0_y1(p_y_zt_dist, q_y_xt_dist, q_z_tyx_dist, p_x_z_dist, torch.tensor(xte).cuda(),
                               torch.tensor(tte).cuda())

            yc0, yc1 = get_control_y0_y1(p_y_zt_dist, q_y_xt_dist, q_z_tyx_dist, p_x_z_dist, torch.tensor(xte).cuda(),
                                         torch.tensor(tte).cuda())

            print("ATE: %f" % np.mean(y1-y0))
            print("ATT: %f" % np.mean(yt1 - yt0))
            print("ATC: %f" % np.mean(yc1 - yc0))
            rmses_test = evaluator_test.y_errors(y0, y1)
            # print('Training set - ite: %f, ate: %f, pehe: %f' % score_train)
            print('test set - rmse factual: %f' % rmses_test)
            rmses_tests.append(rmses_test)
            ...
            plt.savefig('/home/hag007/Desktop/out_1_cmap.png')
            ...
            for key, value in loss.items():
                ...
                plt.title(key)
                subidx += 1
            plt.savefig('/home/hag007/Desktop/out_2_cmap.png')

            fig, ax = plt.subplots(1, 1)
            # ax_2 = ax.twinx()
            ax.plot(np.arange(len(rmses_trains)), rmses_trains, color='red')
            ax.plot(np.arange(len(rmses_tests)), rmses_tests, color='blue')

            plt.savefig("/home/hag007/Desktop/rmse_cmap.png")
