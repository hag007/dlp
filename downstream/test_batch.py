...
def plot(model, test_loader, device, suffix, path_to_save, dataset_names, colormap, bg_color):
    ...
    for a in label_unique:
        plt.scatter([np.mean([xs[i] for i, b in enumerate(labels) if a==b])],[np.mean([ys[i] for i, b in enumerate(labels) if a==b])], s=2000, c=colorlist_unique[a], cmap=colormap, alpha=0.5)
...
def main(model,use_z, fraction, epoch_checkpoint=300, suffix=""):
    ...
    dataset_names=[a for i, a in enumerate(constants.ICGC_DATASETS_NAMES) if constants.ICGC_DATASETS_INCLUDED[i]]
    dataset= cmap_datasets.CMAPDataset(genes, genes_name, dataset_names, "icgc")
    ...
    with torch.no_grad():
        ...
        colormap = cm.terrain
        patches_icgc=plot(encoder, testloader, device, suffix + "_icgc", path_format_to_save.format(epoch_checkpoint), constants.ICGC_DATASETS_NAMES, colormap, 'blue')
...
if __name__=="__main__":
    ...
    epoch_checkpoints=[1100] # [50, 100, 150,200,250,300]
    suffix=""
